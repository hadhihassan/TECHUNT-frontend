                    ...
                    currentPosts.map((post: Project, index: number) => (
                        <div className="w-full mt-5 " key={index} onClick={() => handleShowJobPostDetails(index)}>
                            <div className="m-3 mt-5 w-full">
                                ...
                                <p className='font-sans text-gray-800 mt-1  font-semibold text-sm '>{post.TimeLine} - {post.Expertiselevel} - Est. Budget: ${post.Amount} - Posted  {formatMongoDate(post.createdAt)} -  {post?.WorkType} type</p>
