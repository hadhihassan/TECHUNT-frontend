import { ChangeEvent, useState } from "react"
import { useSelector, useDispatch } from "react-redux";
import { ROOTSTORE } from "../../../../../redux/store";
import { useSocketContext } from "../../../../../context/socketContext";
import { Socket } from "socket.io-client";
import { setMessages } from "../../../../../redux/Slice/conversationsSlice";


const SendMessageInput = () => {
    const [message, setMessage] = useState<string>("");

    const dispatch = useDispatch()

    const { socket }: { socket: Socket } = useSocketContext()as { socket: Socket };

    const isSelectedConversation = useSelector((state: ROOTSTORE) => state.conversation.selectedConversations)
    const myconversation = useSelector((state: ROOTSTORE) => state.conversation)
    const conversation = useSelector((state: ROOTSTORE) => state.conversation.selectedConversations)
    const { id } = useSelector((state: ROOTSTORE) => state.signup)


    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
        if (message.trim() !== "") {
            socket.emit("messageSend", {
                senderId: id,
                message,
                receiverId: conversation?.participants[0]._id
            })
            dispatch(setMessages([...myconversation.messages, {
                message,
                senderId: id,
                receiverId: conversation?.participants[0]._id,
                createdAt: new Date(),
                updatedAt: new Date(),
            }]));
        }
        setMessage("");
    };

    const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
        setMessage(e.target.value);
    };

    return <>
        {
            isSelectedConversation && <form onSubmit={handleSubmit}>
                <div
                    className="flex flex-row items-center mb-20 h-16 rounded-xl bg-white w-full px-4 border shadow-xl "
                >
                    <div>
                    </div>
                    <div className="flex-grow ml-4 ">
                        <div className="relative w-full">
                            <input
                                value={message}
                                onChange={handleChange}
                                type="text"
                                className="flex w-full border rounded-xl focus:outline-none focus:border-indigo-300 pl-4 h-10"
                            />
                            <button
                                className="absolute flex items-center justify-center h-full w-12 right-0 top-0 text-gray-400 hover:text-gray-600"
                            >
                                <svg
                                    className="w-6 h-6"
                                    fill="none"
                                    stroke="currentColor"
                                    viewBox="0 0 24 24"
                                    xmlns="http://www.w3.org/2000/svg"
                                >
                                    <path
                                        stroke-linecap="round"
                                        stroke-linejoin="round"
                                        stroke-width="2"
                                        d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"
                                    ></path>
                                </svg>
                            </button>
                        </div>
                    </div>
                    <div className="ml-4 shadow-xl ">
                        <button
                            className=" flex items-center justify-center bg-red-500 hover:bg-red-600 rounded-xl text-white px-4 py-1 flex-shrink-0"
                        >
                            <span >Send</span>
                            {/* {
                                
                                loading ? <div className=" m-1">
                                    <svg aria-hidden="true" className="w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor" />
                                        <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill" />
                                    </svg>
                                </div> : <span className="ml-2">
                                    <svg
                                        className="w-4 h-4 transform rotate-45 -mt-px"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                        xmlns="http://www.w3.org/2000/svg"
                                    >
                                        <path
                                            stroke-linecap="round"
                                            stroke-linejoin="round"
                                            stroke-width="2"
                                            d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8"
                                        ></path>
                                    </svg>
                                </span>
                                } */}
                        </button>
                    </div>
                </div>
            </form>
        }
    </>
}

export default SendMessageInput